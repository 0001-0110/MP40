    ...
    public class MvcMapperProfile : BijectiveProfile
    {
        public MvcMapperProfile()
        {
            ...
            CreateBijectiveMap<BLL.Models.Video, Video>(source => source
                ...
                // We use the custom mapping for images
                .ForMember(destination => destination.Image, source => source.MapFrom(source => ImageUtility.ToMvcImage(source.Image))),
                source => source
                .ForMember(destination => destination.Image, source => source.MapFrom(source => ImageUtility.ToBllImage(source.Image))));
